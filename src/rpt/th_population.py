...
"""
빅데이터_상권_분석 : 상권_생활인구
분석 결과 1 : 2018Q4에서 2019Q1으로 넘어가는 시기에 급격한 생활 인구 상승. 2020 Q1부터 지속적으로 조금씩 하향.
분석 결과 2 : 구별로 확인한 결과,
구별 총 생활 인구 수 & 구별 남성 생활 인구 수는 강남구, 관악구, 중구 순으로 많음, 노원구, 도봉구, 강북구가 가장 적음
...
분기별_생활인구['총_생활인구_수'] = 분기별_생활인구['총_생활인구_수'] / 10000000 #천 만
...
ax1.set_title("분기별 생활인구(천만명)", fontsize=15)
ax1.set_ylabel('생활인구(천만명)')
...
ax2.set_title("분기별 남성 & 여성 생활인구(천만명)", fontsize=15)
ax2.set_ylabel('생활인구(천만명)')
...
fig.suptitle('분기별 생활인구(천만명)', fontsize=20)
