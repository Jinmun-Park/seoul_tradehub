...
2019년 1분기에서 2019년 2분기로 넘어갈 때 어디가 가장 많이 줄었는지 파악 
-> 시군구: 중구, 영등포구, 종로구, 마포구, 용산구 등이 가장 많이 감소하였다.
-> 상권: 명동 남대문 북창동 다동 무교동 관광특구, 종로구청, 여의도역, 구로디지털1단지, 을지로3가역 가장 많이 감소
분석 결과 2 : 구별로 확인한 결과,
...
# 분기별 직장인구 왜 줄었는지? 2019년 1분기 vs 2019년 2분기
상권_직장인구_2019_1 = 상권_직장인구[상권_직장인구['분기_코드'].str.contains("2019Q1")]
상권_직장인구_2019_2 = 상권_직장인구[상권_직장인구['분기_코드'].str.contains("2019Q2")]

# 상권_직장인구_2019_1 = 상권_직장인구_2019_1.groupby(['상권_코드_명']).agg(총_직장_인구_수_1=('총_직장_인구_수', 'sum'))
# 상권_직장인구_2019_2 = 상권_직장인구_2019_2.groupby(['상권_코드_명']).agg(총_직장_인구_수_2=('총_직장_인구_수', 'sum'))

상권_직장인구_2019_1 = 상권_직장인구_2019_1.groupby(['시군구']).agg(총_직장_인구_수_1=('총_직장_인구_수', 'sum'))
상권_직장인구_2019_2 = 상권_직장인구_2019_2.groupby(['시군구']).agg(총_직장_인구_수_2=('총_직장_인구_수', 'sum'))

# 상권_직장인구_1920 = pd.merge(상권_직장인구_2019_1, 상권_직장인구_2019_2, on='상권_코드_명')
상권_직장인구_1920 = pd.merge(상권_직장인구_2019_1, 상권_직장인구_2019_2, on='시군구')
상권_직장인구_1920['직장 인구 차이(2-1)'] = 상권_직장인구_1920['총_직장_인구_수_2']-상권_직장인구_1920['총_직장_인구_수_1']
상권_직장인구_1920 = 상권_직장인구_1920.sort_values(by='직장 인구 차이(2-1)', ascending=True)
