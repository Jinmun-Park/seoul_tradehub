...
직장인구_pct = 상권_직장인구[상권_직장인구['분기_코드'].str.contains("2019Q1|2019Q2")]
직장인구_pct = 직장인구_pct.groupby(['기준_년_코드','기준_분기_코드','분기_코드','상권_코드_명']).agg(
    총_직장_인구_수=('총_직장_인구_수', 'sum'),
    남성_직장_인구_수=('남성_직장_인구_수', 'sum'),
    여성_직장_인구_수=('여성_직장_인구_수', 'sum')).reset_index()

직장인구_pct['직장인구_pct_change_상권'] = 직장인구_pct.sort_values(['분기_코드']).groupby(['상권_코드_명']).총_직장_인구_수.pct_change()
직장인구_pct = 직장인구_pct.sort_values(by='직장인구_pct_change_상권', ascending=True)

# pct change로 확인해보기 _ 구별
직장인구_pct = 상권_직장인구[상권_직장인구['분기_코드'].str.contains("2019Q1|2019Q2")]
직장인구_pct = 직장인구_pct.groupby(['기준_년_코드','기준_분기_코드','분기_코드','시군구']).agg(
    총_직장_인구_수=('총_직장_인구_수', 'sum'),
    남성_직장_인구_수=('남성_직장_인구_수', 'sum'),
    여성_직장_인구_수=('여성_직장_인구_수', 'sum')).reset_index()

직장인구_pct['직장인구_pct_change_시군구'] = 직장인구_pct.sort_values(['분기_코드']).groupby(['시군구']).총_직장_인구_수.pct_change()
직장인구_pct = 직장인구_pct.sort_values(by='직장인구_pct_change_시군구', ascending=True)
